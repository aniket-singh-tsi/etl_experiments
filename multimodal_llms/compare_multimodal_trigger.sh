...
CONFIG_FILES["language"]=$mydir/evaluation-config.language.json
CONFIG_FILES["visionlanguage"]=$mydir/evaluation-config.visionlanguage.json

#============================================================
CONFIG_FILE="evaluation-config.json"
for mode in "${modes[@]}"; do
    
    CONFIG_FILE_=${CONFIG_FILES["${mode}"]}
    cp ${CONFIG_FILE_} $mydir/../../$CONFIG_FILE
    # rm evaluation-config.json
    ...
    echo "======================="
    echo " "
    cmd="mv ${mydir}/tmp_results/$etlJobId/evaluation-summary-data/overall_evaluation_summary/2025-02-18/ETLSimpleLlmEvaluator_summary.csv ${mydir}/results/ETLSimpleLlmEvaluator_summary_mode_${mode}.csv"
    echo $cmd
    $cmd
done
#============================================================